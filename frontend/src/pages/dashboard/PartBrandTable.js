...
const PartBrandTable = () => {
    ...
    const loadData = async () => {
        try {
            const res = await API.get('/part-brands');
            setBrands(res.data);
            setLoading(false);
        } catch (err) {
            console.error("Parça markaları yüklenemedi", err);
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await API.delete(`/part-brands/${id}`);
    ...
    const handleUpdate = async () => {
        try {
            await API.put(`/part-brands/${editMode._id}`, editMode);
            setEditMode(null);
            loadData();
            alert("✅ Üretici güncellendi!");
        } catch (err) {
        ...
        <div style={tableStyles.container}>
            ...
            <div style={tableStyles.filterBar}>
                <div style={tableStyles.filterGroup}>
                    <input 
                        placeholder="Üretici (Bosch, Valeo...) ara..." 
                        style={tableStyles.filterInput}
                        value={searchTerm}
                        onChange={e => {setSearchTerm(e.target.value); setCurrentPage(1);}}
                    />
                    <div style={{marginLeft: 'auto', fontSize: '14px', color: '#666'}}>
                        Toplam: <strong>{filteredBrands.length}</strong> Üretici
                    </div>
                </div>
            </div>
                ...
                <table style={tableStyles.table}>
                    <thead>
                        <tr style={tableStyles.thRow}>
                            ...
                            <th style={tableStyles.th}>Üretici Adı</th>
                            <th style={tableStyles.th}>Sistem Kaydı</th>
                    ...
                    <tbody>
                        {currentItems.map((brand) => (
                            <tr key={brand._id} style={tableStyles.tr}>
                                ...
                                <td style={tableStyles.td}>{brand.createdAt ? new Date(brand.createdAt).toLocaleDateString('tr-TR') : '-'}</td>
